...
Coolifyでの初回デプロイ後、割り当てられたドメインにアクセスすると原因不明のエラーに。コンテナの起動状況・DNS・リバースプロキシの稼働状況を一つずつ切り分けて調査した結果、原因はドメインの設定が`http://`のままで、HTTPS用の証明書(Let's Encrypt)が発行されていなかったことでした。ドメイン設定を`https://`に変更することで解決し、以降トラブルシューティングでは「見えているエラーメッセージ」だけでなく、その手前の設定(プロトコル・証明書の発行状況)まで疑う習慣がつきました。
